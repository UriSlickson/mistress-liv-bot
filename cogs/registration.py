...
class RegistrationCog(commands.Cog):
    ...
    @app_commands.command(name="autoregisterall", description="[Admin] Automatically register all members with team roles")
    @app_commands.default_permissions(administrator=True)
    @app_commands.describe(
        sync_helmets="Also sync helmet emojis to nicknames (default: True)"
    )
    async def auto_register_all(self, interaction: discord.Interaction, sync_helmets: bool = True):
        ...
        try:
            ...
            conn = sqlite3.connect(self.db_path)
            cursor = conn.cursor()
            cursor.execute("SELECT team_id, user_discord_id FROM teams WHERE user_discord_id IS NOT NULL")
            existing_registrations = {row[0]: row[1] for row in cursor.fetchall()}
            conn.close()
        except:
        ...
        response.append(f"✅ **Newly registered:** {registered}")
        response.append(f"🟢 **Already registered:** {already_registered}")
        if sync_helmets:
            response.append(f"🏈 **Helmets synced:** {helmets_synced}")
        if errors > 0:
        ...
        if results:
            ...
            if len(results) > 15:
                response.append(f"... and {len(results) - 15} more")
